...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize promotions slideshow
    fetchActivePromotions().then(promotions => {
        const slideshowHtml = generateSlideshowHtml(promotions);
        const wrapper = document.querySelector('.roomBlockSelectionWrapper');
        if (wrapper) {
            // Remove existing static slideshow if present
            const existingSlideshow = wrapper.nextElementSibling;
            if (existingSlideshow && existingSlideshow.classList.contains('w-full')) {
                existingSlideshow.remove();
                console.log('Removed existing static slideshow');
            }

            // Add new slideshow
            const slideshowContainer = document.createElement('div');
            slideshowContainer.className = 'w-full';
            slideshowContainer.innerHTML = `
                <div class="swiper promotions-swiper">
                    <div class="swiper-wrapper">${slideshowHtml}</div>
                    <div class="swiper-pagination"></div>
                    <div class="swiper-button-prev"></div>
                    <div class="swiper-button-next"></div>
                </div>
            `;
            wrapper.insertAdjacentElement('afterend', slideshowContainer);
            console.log('Promotions slideshow container added');
            ...
            if (typeof Swiper !== 'undefined') {
                ...
                console.log('Swiper initialized for promotions slideshow');
            } else {
                console.error('Swiper library not loaded');
            }
        } else {
            console.error('Could not find .roomBlockSelectionWrapper');
        }
    }).catch(error => {
        console.error('Error in promotions slideshow setup:', error);
    });
});